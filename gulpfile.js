...
gulp.task("grammar", function (cb) {
  ...
  exec(`${local_pegjs} ${Grammar_pegjs} ${Grammar_js}`, function (
    error,
    stdout,
    stderr
  ) {
    ...
    let modified_grammar = grammar.replace(
...
gulp.task("dist", gulp.series("babel", "browserify", "uglify"));
// gulp.task("dist", gulp.series("babel", "test", "browserify", "uglify"));
